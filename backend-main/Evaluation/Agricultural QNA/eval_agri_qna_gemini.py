...
def get_gemini_response(question, max_retries=3):
    """Get response from Gemini 2.0 Flash for agriculture questions"""
    system_prompt = """You are an expert in agriculture, farming practices, and crop management. 
    Provide clear, accurate, and practical answers to agriculture-related questions. 
    Focus on scientific information and best practices. Keep your answers concise but informative."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            generation_config = genai.types.GenerationConfig(
                ...
                top_p=0.9,
                top_k=40,
                max_output_tokens=400,  # Moderate length for detailed but concise answers
                stop_sequences=None,
            )
            ...
            if response.candidates and response.candidates[0].content.parts:
                ...
                print(f"Response blocked or empty for question: {question[:50]}...")
...
def evaluate_round_comprehensive(data, num_samples=10):
    ...
    for i, example in enumerate(samples, 1):
        ...
        # Get model response with rate limiting consideration
        model_answer = get_gemini_response(question)
...
def main():
    ...
    print(f"\n🔧 CONFIGURATION:")
    print(f"🤖 Model: Google Gemini 2.0 Flash Experimental")
